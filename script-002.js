...
canvas.addEventListener('click', (e) => {
    ...
    dots.forEach(dot => {
        if (Math.hypot(mouseX - dot.x, mouseY - dot.y) < 10) {
            if (!currentCluster) {
                // First dot in a new cluster
                currentCluster = [dot];
                dot.connected = true;
                dot.cluster = currentCluster;
            } else if (dot === currentCluster[0]) {
                // Close the loop by clicking the first dot again
                currentCluster = null; // Break the cluster selection mode
            } else {
                // Add dot to the current cluster
...
function updateDots() {
    ...
    dots.forEach(dot => {
        if (dot.cluster && dot.cluster.length > 2) {
            const cluster = dot.cluster;
            const avgVx = cluster.reduce((sum, d) => sum + d.vx, 0) / cluster.length;
            const avgVy = cluster.reduce((sum, d) => sum + d.vy, 0) / cluster.length;
